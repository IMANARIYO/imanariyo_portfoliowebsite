...
function BlogModalContent ({ handleClose, props }) {
  ...
  const checkLikedStatusAndCheckDisLikedStatus = async () => {
    ...
    try {
      const response = await axios.get(
        `https://myportfolioapi-8vku.onrender.com/Blogs/getBlog/${props._id}`,
        {
          headers: {
  ...
  const handleliking = async () => {
    ...
    try {
      const response = await axios.post(
        `https://myportfolioapi-8vku.onrender.com/Blogs/likeBlog/${props._id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${accessToken}`
  ...
  const handleDislike = async () => {
    ...
    try {
      const response = await axios.post(
        `https://myportfolioapi-8vku.onrender.com/Blogs/dislikeBlog/${props._id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${accessToken}`
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        `https://myportfolioapi-8vku.onrender.com/comments/${props._id}`
      )
  ...
  return (
    <div className='modal-container'>
      <div className='blog-container'>
        ...
        <img
          className='signle-blog-main-img'
          src={image}
          frameBorder='0'
          allowFullScreen
        />
        ...
        <div className='button-group'>
          ...
          <button
            className='blog-button likes flex gap-2 '
          ...
          <button
            className='blog-button dislikes flex gap-2'
        ...
        <div id='comments'>
          ...
          <div className='comments'>
            {comments.map(comment =>
              <div
                className='single-user-comment border border-blue-500'
                key={comment._id}
              >
                {/* Render each comment */}
                <div className='single-user-comment-content  '>
                  <div className='flex '>
                    ...
                    <div>
                      ...
                      <div className='single-user-comment-message'>
                        {comment.comment}
                      </div>
                    </div>
                  </div>
                  <div className='button-group'>
                    <button className='single-comment-like-dislike'>
                      Likes: 0
                    </button>
                    <button className='single-comment-like-dislike'>
                      Dislikes: 0
